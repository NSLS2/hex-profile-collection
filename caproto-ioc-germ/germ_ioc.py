...
EPICS_CAS_AUTO_BEACON_ADDR_LIST=no EPICS_CAS_BEACON_ADDR_LIST=127.0.0.1 python germ_ioc.py -v
"""
import asyncio
import contextvars
import datetime
...
class GeRMSaveIOC(PVGroup):
    ...
    async def stage(self, instance, value):
        if instance.value in [True, "staged"] and value == "staged":
            raise ValueError("The device is already staged. Unstage it first.")
        if value == "staged":
