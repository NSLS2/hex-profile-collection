...
class HDF5Writer:
    def __init__(
        ...
        if "images" not in self.data_grp:
            self.dataset = self.data_grp.create_dataset(
                "images", dataset_shape, dtype=data_type
            )
        else:
    ...
    def save_data(self, image, scan_params=None, metadata=None, metadata_group=None):
        try:
                ...
                for key, value in scan_params.items():
                    ...
                    if param_dataset_name not in self.scan_params_grp:
                        ...
                        param_dset = self.scan_params_grp.create_dataset(
                            param_dataset_name, (self.dataset.shape[0],), dtype=dtype
                        )
                    else:
            ...
            if metadata and metadata_group:
                if len(metadata) != len(metadata_group):
                    raise ValueError(
                        "Length of metadata list must match the "
